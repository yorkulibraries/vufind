...
// assign muler resolver prefix if configured
if (isset($configArray['MULER']['host'])) {
    $interface->assign('mulerHost', $configArray['MULER']['host']);
}
...
function handlePEARError($error)
{
    ...
    foreach ($fatalErrors as $fatal) {
        if (stripos($error->getMessage(), $fatal) !== false) {
            $isFatal = true;
            break;
        }
    }
    ...
    $interface = new UInterface();
    $interface->assign('isFatal', $isFatal);
    ...
    // Assign modal mode if requested
    if ($_REQUEST['modal']) {
    ...
    $level = $isFatal ? PEAR_LOG_EMERG : PEAR_LOG_ERR;
    $logger->log($errorDetails, $level);
