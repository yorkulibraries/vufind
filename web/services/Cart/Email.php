...
class Email extends Bulk
{
    ...
    public function sendEmail($url, $to, $from, $message)
    {
        ...
        // fetch the records from the index and generate the email views for them
        foreach($_POST['ids'] as $id) {
            // Retrieve the record from the index
            if ($record = $this->db->getRecord($id)) {
                $recordDriver = RecordDriverFactory::initRecordDriver($record);
                $emailDetails .= "\n\n-----------------------------------------\n" 
                    . $recordDriver->getEmail();
            }
        }
        ...
        //$interface->assign('__emailDetails', $emailDetails);
        $body = $interface->fetch('Emails/share-link.tpl') . $emailDetails;
