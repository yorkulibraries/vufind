...
class Record extends Action
{
    ...
    public function __construct()
    {
        ...
        if (!$interface->is_cached($this->cacheId)) {
            ...
            if ($this->recordDriver->isFond()) {
                $similar = $this->db->getMoreLikeThis($_REQUEST['id'], 'morelikethisfond');
            } else {
        ...
        $interface->assign('lastSearchQuery', $_SESSION['lastSearchQuery']);
        $interface->assign('lastFilterList', $_SESSION['lastFilterList']);
        $interface->assign('lastSearchType', $_SESSION['lastSearchType']);
        $interface->assign('lastSearchId', $_SESSION['lastSearchId']);
    }
