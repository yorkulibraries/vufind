...
class Edit extends Action
{
    ...
    {
        $tagStr = '';
        $myTagList = $user->getTags($_REQUEST['id'], $listId);
    ...
    {
        $resource = Resource::staticGet('record_id', $_REQUEST['id']);
    ...
    public function launch()
    {
        ...
        $details = $db->getRecord($_REQUEST['id']);
        ...
        $saved = $user->getSavedData($_REQUEST['id']);
        
        // Add tag information
        $savedData = array();
        foreach ($saved as $current) {
            // If we're filtering to a specific list, skip any other lists:
            if (isset($_REQUEST['list_id']) && $current->list_id != $_REQUEST['list_id']) {
                continue;
            }
            $savedData[] = array(
        ...
        $interface->assign(
            'listFilter', isset($_REQUEST['list_id']) ? $_REQUEST['list_id'] : null
        );
        $interface->setPageTitle('Edit Resource');
