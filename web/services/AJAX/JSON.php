...
class JSON extends Action
{
     ...
     * @access public
     */
    public function emailBookBag()
    {
        include_once 'services/BookBag/Email.php';

        if (strlen(trim($_REQUEST['to'])) == 0) {
            return $this->output(
                translate('Recipient is required'), JSON::STATUS_ERROR
            );
        }
    ...
    {
        include_once 'services/MyResearch/EmailList.php';
        ...
        $result = new stdClass();
        $result->modalAlert = $this->modalAlert('Email List', translate('email_success'), 'success');
        return $this->output($result, JSON::STATUS_OK);
    }
    
    /**
     * Save a bookbag to a list.
     *
     * @return void
     * @access public
     */
    public function saveBookBag()
    {
        include_once 'services/BookBag/Save.php';
    ...
    /**
     * Generate the HTML for a new Advanced Search row.
     *
     * @return void
     * @access public
     */
    public function newAdvancedSearchRow()
    {
        global $interface;
        
        $searchObject = SearchObjectFactory::initSearchObject();
        $interface->assign('advSearchTypes', $searchObject->getAdvancedTypes());
        $interface->assign('group', $_REQUEST['group']);
        $html = $interface->fetch('Search/advanced-search-row.tpl');
        $this->output($html, JSON::STATUS_OK);
    }
    
    /**
     * Login with post'ed username and password.
     *
     * @return void
     * @access public
     */
    public function altLogin()
    {
        global $configArray;
        global $interface;

        // AJAX login info is sent with ajax_ prefix
        // we need to remove the prefix before calling login()
        $_POST['username'] = $_POST['ajax_username'];
        $_POST['password'] = $_POST['ajax_password'];
        ...
        // prepare the followup URL
        $result = new stdClass();
        if ($_REQUEST['followupURL']) {
            $result->followup = $_REQUEST['followupURL'];
            if ($_REQUEST['followupQueryString']) {
                $result->followup .= '?' . $_REQUEST['followupQueryString'];
            }
        }
        
        // prepare the logged in panel and event data
    ...
    public function placeRequest() {
        ...
        if (!in_array($requestType, array('Hold', 'ICB', 'InProcess', 'Storage'))) {
            $this->output(translate('Unsupported Request'), JSON::STATUS_ERROR);
            exit;
        }
                
        require_once "services/Record/{$requestType}.php";
        ...
        $service = new $requestType();

        if (!$service->isRequestAllowed()) {
            $this->output('This request is not allowed', JSON::STATUS_ERROR);
        }

        $service->gatherDetails();
        $errors = $service->validate();
        $result = new stdClass();
        if (!empty($errors)) {
            $result->formErrors = $errors;
            $this->output($result, JSON::STATUS_ERROR);
            exit;
        }
        if($service->process()) {
            $result = new stdClass();
            $result->modalAlert = $this->modalAlert($requestType . ' Request', translate('Request placed'), 'success');
            return $this->output($result, JSON::STATUS_OK);
        }
        return $this->output(translate('Error processing request'), JSON::STATUS_ERROR);
    }
    ...
    private function _filterSuppressedLocations($record)
    {
        ...
        foreach ($record as $current) {
            if (!in_array($current['location'], $hideHoldings) || $current['item_type'] == 'IN-PROCESS') {
                ...
                if (in_array($current['location'], array('Lost', 'Missing')) 
    ...
    public function getItemStatuses()
    {
        ...
        // Done
        $maxAge = isset($configArray['Caching']['ajax_ils_status_expiry']) 
            ? $configArray['Caching']['ajax_ils_status_expiry'] : 0;
        return $this->output($statuses, JSON::STATUS_OK, $maxAge);
    }
    ...
    public function saveRecord()
    {
        ...
        if ($list->find(true)) {
            $result = new stdClass();
            $result->modalAlert = $this->modalAlert('Save Record', translate('Record saved'), 'success');
            return $this->output($result, JSON::STATUS_OK);
        }
        return $this->output('Error occurred while saving record', JSON::STATUS_ERROR);
    }
    
    private function modalAlert($title, $message, $type) 
    {
        global $interface;
        $interface->assign('alertTitle', $title);
        $interface->assign('alertMessage', $message);
        $interface->assign('alertType', $type);
        return $interface->fetch('AJAX/modal-alert.tpl');
    }
    ...
    public function emailRecord()
    {
        ...
        $result = new stdClass();
        $result->modalAlert = $this->modalAlert('Email Record', translate('email_success'), 'success');
        return $this->output($result, JSON::STATUS_OK);
    }
    ...
    public function smsRecord()
    {
        ...
        $result = new stdClass();
        $result->modalAlert = $this->modalAlert('SMS Record', translate('sms_success'), 'success');
        return $this->output($result, JSON::STATUS_OK);
    }
    ...
    public function tagRecord()
    {
        ...
        $result = new stdClass();
        $result->modalAlert = $this->modalAlert('Add Tag', translate('Tag added'), 'success');
        return $this->output($result, JSON::STATUS_OK);
    }
    ...
    public function commentRecord()
    {
        ...
        // get the updated list of comments as HTML
        ...
        // send result
        $result = new stdClass();
        $result->modalAlert = $this->modalAlert('Add your comment', translate('Comment added'), 'success');
        $event = new stdClass();
        $event->type = 'commentadded.vufind';
        $event->data = new stdClass;
        $event->data->list = $html;
        $event->data->message = translate('Comment added');
        $result->events = array($event);
        return $this->output($result, JSON::STATUS_OK);
    }
    ...
    public function deleteRecordComment()
    {
        ...
        if (!UserComments::deleteComment($_REQUEST['id'], $user)) {
            return $this->output(
                translate('An error has occurred'), JSON::STATUS_ERROR
            );
        }
        ...
        // get the updated list of comments as HTML
        $interface->assign('id', $_REQUEST['recordId']);
        UserComments::assignComments($_REQUEST['recordId']);
        $html = $interface->fetch('Record/view-comments-list.tpl');
        
        $result = new stdClass();
        $event = new stdClass();
        $event->type = 'commentdeleted.vufind';
        $event->data = new stdClass();
        $event->data->message = translate('Comment deleted');
        $event->data->list = $html;
        $result->events = array($event);
        return $this->output($result, JSON::STATUS_OK);
    }
    ...
    public function emailSearch()
    {
        ...
        $result = new stdClass();
        $result->modalAlert = $this->modalAlert('Email This Search', translate('email_success'), 'success');
        return $this->output($result, JSON::STATUS_OK);
    }
    ...
    public function getResolverLinks()
    {
        ...
        $openUrl = isset($_GET['openurl']) ? $_GET['openurl'] : array();
        $openurls = is_array($openUrl) ? $openUrl : array($openUrl);
        $issns = isset($_GET['issn']) ? $_GET['issn'] : array();
        ...
        // resolve each openurl, stop when some link is found
        $result = array();
    ...
    {
        include_once 'services/Feedback/Submit.php';
        
        $result = Submit::doSubmit();
        if (PEAR::isError($result)) {
            return $this->output(
                translate($result->getMessage()), JSON::STATUS_ERROR
            );
        ...
        $result = new stdClass();
        $result->modalAlert = $this->modalAlert('Feedback', translate('feedback_submitted'), 'success');
        return $this->output($result, JSON::STATUS_OK);
    }
     ...
     * @access public
     */
    protected function output($data, $status, $maxAge=false)
    {
        if (!$maxAge) {
