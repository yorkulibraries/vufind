...
class Solr implements IndexEngine
{
    ...
    public function ping($raiseError = false)
    {
        $test_url = $this->host . "/admin/ping";
        $test_client = new Proxy_Request(null, array('timeout' => 5));
        ...
        if (!PEAR::isError($result)) {
        	...
        	$responseBody = $test_client->getResponseBody();
        	if (strpos($responseBody, '<str name="status">OK</str>') === false) {
        	    if ($raiseError) {
        		    PEAR::raiseError('Solr index is offline.', $test_client->getResponseCode());
        	    } else {
        	        return false;
        	    }
        	}
     ...
     * @access public
     */
    public function getMoreLikeThis($id, $qt='morelikethis')
    {
        // Query String Parameters
        $options = array('q' => "id:$id", 'qt' => $qt);
        $result = $this->_select('GET', $options);
        if (PEAR::isError($result)) {
            PEAR::raiseError($result);
        }
    ...
    public function search($query, $handler = null, $filter = null, $start = 0,
        ...
        if ($sort && !empty($sort)) {
            ...
            $options['sort'] = 'score desc, _docid_ desc';
        }
