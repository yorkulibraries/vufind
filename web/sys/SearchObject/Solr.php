...
class SearchObject_Solr extends SearchObject_Base
{
    ...
    public function init()
    {
        ...
        } else if ($module == 'BookBag') {
            $this->spellcheck = false;
            $this->searchType = 'bookbag';
        }
    ...
    protected function getBaseUrl()
    {
        ...
        if ($this->searchType == 'author') {
            ...
            return $this->serverUrl . '/MyResearch/MyList/' .
                urlencode($_GET['id']) . '?';
        } else if ($this->searchType == 'bookbag') {
            return $this->serverUrl."/BookBag/Home?";
        }
    ...
    public function buildRSS($result = null)
    {
        ...
        if (isset($_REQUEST['json'])) {
        	...
        	return json_encode($result['response']['docs']);
        } else {
        	// XML HTTP header
        ...
        if ($serializer->serialize($result)) {
            ...
            $xmlResults = preg_replace("/\x1D/", "",  $xmlResults);
            $xmlResults = preg_replace("/\x1E/", "",  $xmlResults);
            $xmlResults = preg_replace("/\x1F/", "",  $xmlResults);
        }
