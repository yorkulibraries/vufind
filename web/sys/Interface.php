...
class UInterface extends Smarty
{
    ...
    public function UInterface()
    {
        ...
        if (!empty($configArray['GoogleAnalytics']['id'])) {
            $this->assign('gaId', $configArray['GoogleAnalytics']['id']);
        }
        
        $this->assign(
            'coversOnLeft', 
            !isset($configArray['Site']['coversOnLeft'])
            ? false : $configArray['Site']['coversOnLeft'] 
        );
        
        $this->assign('smarty_template_dir', $this->template_dir);
    }
