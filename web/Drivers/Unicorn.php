...
class Unicorn implements DriverInterface
{
    ...
    */
    ...
    public function placeHold($holdDetails)
    {
        ...
        $matches = array();
        preg_match('/\^MN([0-9]+)/', $response, $matches);
        if (isset($matches[1])) {
            $status = $matches[1];
            $sysMessage = isset($this->ilsConfigArray['ApiMessages'][$status])
                ? $this->ilsConfigArray['ApiMessages'][$status] : $status;
            if ($status == '209') {
        ...
        return array('success' => false, 'sysMessage' => $response);
    }
    ...
    public function getMyHolds($patron)
    {
        ...
        foreach ($lines as $item) {
            ...
            $date_available, $date_available_expires, $comment, $barcode, $callnum)
                = explode('|', $item);
