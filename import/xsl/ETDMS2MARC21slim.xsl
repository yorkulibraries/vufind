...
<xsl:stylesheet version="1.0" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns="http://www.loc.gov/MARC21/slim" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="dc oai_etdms"
	...
	<xsl:template match="/">
		<record xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.loc.gov/MARC21/slim http://www.loc.gov/standards/marcxml/schema/MARC21slim.xsd" >
			<xsl:element name="leader">
				<xsl:variable name="type" select="oai_etdms:type"/>
			...
			<xsl:for-each select="//oai_etdms:contributor">
			...
			<xsl:for-each select="//oai_etdms:coverage">
			...
			<xsl:for-each select="//oai_etdms:date">
			...
			<xsl:for-each select="//oai_etdms:description">
			...
			<xsl:for-each select="//oai_etdms:identifier">
			  <xsl:choose>
			    <xsl:when test="starts-with(., 'http')">
			      <xsl:if test="contains(., '/bitstream/')">
  			      <datafield tag="856" ind1="4" ind2="0">
      					<subfield code="u">
      						<xsl:value-of select="."/>
      					</subfield>
      					<subfield code="q">
      					  <xsl:value-of select="//oai_etdms:format"/>
      					</subfield>
      				</datafield>
  			    </xsl:if>
  			  </xsl:when>
			...
			<xsl:for-each select="//oai_etdms:language">
			...
			<xsl:for-each select="//oai_etdms:publisher">
				<datafield tag="260" ind1=" " ind2=" ">
					<subfield code="b">
						<xsl:value-of select="."/>
					</subfield>
			...
			<xsl:for-each select="//oai_etdms:relation">
			...
			<xsl:for-each select="//oai_etdms:rights">
			...
			<xsl:for-each select="//oai_etdms:source">
			...
			<xsl:for-each select="//oai_etdms:subject">
			...
			<xsl:for-each select="//oai_etdms:title[1]">
			...
			<xsl:for-each select="//oai_etdms:title[position()>1]">
			...
			<xsl:for-each select="//oai_etdms:type">
			...
			<xsl:for-each select="//oai_etdms:degree">
				<datafield tag="502" ind1=" " ind2=" ">
					<subfield code="a"><xsl:value-of select="oai_etdms:name/text()"/> (<xsl:value-of select="oai_etdms:level/text()"/>) -- <xsl:value-of select="oai_etdms:grantor/text()"/>.</subfield>
				</datafield>
				<datafield tag="710" ind1=" " ind2=" ">
				  <subfield code="b"><xsl:value-of select="oai_etdms:discipline/text()"/></subfield>
				</datafield>
			</xsl:for-each>
		</record>
	</xsl:template>
