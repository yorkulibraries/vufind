...
public class PreIndexProcess implements Runnable {
	...
	static String insertResolverIdSql = 
		"insert into resolver_ids (record_id, number, source) "
		+ "values (?, ?, ?) on duplicate key update id=id";
	
	static String insertISSNSql = 
			"insert into issns (record_id, number, source) "
			+ "values (?, ?, ?) on duplicate key update id=id";
	
	static String insertMARCSql = 
			"insert into marc (record_id, marc) "
			+ "values (?, ?) on duplicate key update id=id";
	
	public PreIndexProcess(String file, String source, Connection db) throws SQLException {
		this.file = file;
		this.source = source;
		this.db = db;
			...
			for (String url : urls) {
				for (String prefix : resolverPrefixes) {
					if (url.startsWith(prefix)) {
						...
						logger.debug("resolver ID number: " + resolverId + " for record ID: " + recordId);
	...
	private void saveResolverId(String recordId, String resolverId) throws SQLException {
	...
	public void run() {
		...
		logger.info("Processed " + count + " records. source=" + source + ", file=" + file);
	}
}
