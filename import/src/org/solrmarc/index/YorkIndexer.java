...
public class YorkIndexer extends VuFindIndexer {
	...
	public Set<String> getSuppressedBy(Record record, String source)
		...
		if ("muler".equalsIgnoreCase(source)) {
			...
			for (String url : urls) {
				...
				if (resolverId != null && resolverIDsInSirsi.contains(resolverId)) {
					stmt.setString(1, "catalog");
					stmt.setString(2, resolverId);
	...
	private String extractResolverId(String url) {
		String id = null;
		if (url.indexOf("http://www.library.yorku.ca/eresolver/?id=") != -1) {
			id = url.substring("http://www.library.yorku.ca/eresolver/?id="
					.length());
		} else if (url.indexOf("http://www.library.yorku.ca/e/resolver/id/") != -1) {
			id = url.substring("http://www.library.yorku.ca/e/resolver/id/"
					.length());
		}
		return id;
	...
	public Set<String> getLocation(Record record) {
		...
		// check if item is available online, if so then add INTERNET as a
		// location
		Set<String> issns = getISSNs(record);
		for (String issn : issns) {
	...
	private static void loadShelvingKeys() {
		...
		logger.info("Loading shelving keys map from "
	...
	private static void loadItemsLocations() {
		...
		logger.info("Loading items locations map from "
				+ tmpFile.getAbsolutePath());
		try {
			if (tmpFile.exists() && tmpFile.length() > 0) {
				BufferedReader reader = new BufferedReader(new FileReader(
