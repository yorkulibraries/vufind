...
public class VideoInfoMixin extends SolrIndexerMixin
{
    ...
    public static Set<String> getVideoDirectorsFromTextField(String responsibility, boolean greedy)
    {
        ...
        responsibility1 = responsibility1.replaceAll("[Tt]asritÌ£ u-vimui", "didrector");//hebrew
...
//        }
        ...
        responsibility1 = responsibility1.replaceAll("[Yy]oÌ†nchÊ»ul", "didrector");//korean
        if (!responsibility1.equals(responsibility))
        {
            responsibility = responsibility1;
        }
         ...
         //           (loop == 5 && part.matches(".*maybe by.*")) )
                ...
                //    part = part.replaceAll("\\[sic[.][]]", "");
                    ...
                    else if (part.matches(".*[Dd]irect(ed|ion).*?by.*")|| part.matches(".*a film by.*") || part.matches(".*maybe by.*"))
                    {
                        ...
                        part = part.replaceAll("directified by[ a-z,]*(([\"]?([A-Z]|\\p{Lu}|[*Ê»]|\\p{M})[^ ]*[\"]?[,]?[ ]*|[ ]?&[ ]|zur |von |van |de[rl]?[ ]?|the |d[']|al-|da-)+).*", "$1");
                    ...
                    else if (part.matches(".*[Dd]irector[^a-rt-z\'].*[A-Z].*"))
                    {
                        ...
                        part = part.replaceFirst("director[^A-Z]* ([Ê»*]?[A-Z])", "director= $1");
                        ...
                        part = part.replaceAll("= [^(]*[)], ", ": ");
                        if (greedy)
                            part = part.replaceAll("director=[ a-z,]*(([\"]?([A-Z]|\\p{Lu}|[*Ê»]|\\p{M})[^ ]*[\"]?[,]?[ ]*|[ ]?&[ ]|von |zur |van |de[rl]?[ ]?|the |in |d[']|al-|da-)+)[^|&]*", "$1");
                        else // strict
                            part = part.replaceAll("director=[ a-z,]*(([\"]?([A-Z]|\\p{Lu}|[*Ê»]|\\p{M})[^ ]*[\"]?[,]?[ ]*|[ ]?&[ ]|von |zur |van |de[rl]?[ ]?|the |in |d[']|al-|da-)+)( [a-z].*|$)", "$1");
                        part = part.replaceAll("^([A-Z][^ .]+) & ([A-Z][^ .]+) ([A-Z][^ .]+)", "$1 $3 & $2 $3");
                        part = part.replaceAll("([,][ ]?|[ ]?[|&][ ]?)", "|");
                        part = part.replaceAll("[#]", ",");
                        part = part.replaceAll("[ ][ ]+", " ");
