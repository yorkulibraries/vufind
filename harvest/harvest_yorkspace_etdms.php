...
class HarvestOAI
{
    ...
    {
        // Make the OAI-PMH request to get the identifiers
        $response = $this->_sendRequest('ListIdentifiers', $params);
        foreach($response->ListIdentifiers->header as $header) {
            $identifiers[] = (string) $header->identifier;
        }
        
        // identifiers to skip 
        $skipList = array('oai:yorkspace.library.yorku.ca:10315/28211');
        
        // get each record by identifier
        foreach ($identifiers as $identifier) {
            if (in_array($identifier, $skipList)) {
                continue;
            }
            $p = array(
            ...
            $response = $this->_sendRequest('GetRecord', $p);
            // Save the record from the response:
            if ($response->GetRecord->record) {
                $this->_processRecords($response->GetRecord->record);
            }
        }
        return false;
    }
